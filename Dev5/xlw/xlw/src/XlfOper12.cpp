...
{
    lpxloper_ = TempMemory::GetMemory<XLOPER12>();
...
{
    int err = XlfExcel::Instance().XlfExcel::Instance().Call12(xlFree, NULL, 1, lpxloper_);
    if (err != xlretSuccess)
...
int xlw::XlfOper12::Coerce(short type, XlfOper12& result) const
{
    ...
    int xlret = XlfExcel::Instance().Call12(xlCoerce, result.lpxloper_, 2, lpxloper_, xlType.lpxloper_);
    return xlret;
}
...
int xlw::XlfOper12::ConvertToWstring(std::wstring &w) const throw()
{
    ...
    {
        size_t n = lpxloper_->val.str[0];
        wchar_t *s = TempMemory::GetMemory<wchar_t>(n+1);
...
xlw::XlfOper12& xlw::XlfOper12::Set(const CellMatrix& cells)
{
    ...
    lpxloper_->val.array.lparray = TempMemory::GetMemory<XLOPER12>(r*c);
...
xlw::XlfOper12& xlw::XlfOper12::Set(const char *value)
{
    if (lpxloper_)
    {
        ...
        lpxloper_->val.str = TempMemory::GetMemory<XCHAR>(len+2);
...
xlw::XlfOper12& xlw::XlfOper12::SetElement(RW r, COL c, const XlfOper12 &value)
{
    ...
    if (value.lpxloper_->xltype == xltypeNum) {
        ...
        size_t n = value.lpxloper_->val.str[0] + 1;
        element.val.str = TempMemory::GetMemory<wchar_t>(n);
        memcpy(element.val.str, value.lpxloper_->val.str, n * 2);
